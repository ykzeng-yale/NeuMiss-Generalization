"""Focused experiment: Effect of residual connections and skip connections in NeuMiss+."""
...
for variant in ['A', 'B', 'C']:
    for res in [False, True]:
        for depth in [3, 5, 7]:
            tag = '_res' if res else ''
            cfg = {'name': f'NM+{variant}_gelu_d{depth}{tag}', 'variant': variant, 'depth': depth,
                   'activation': 'gelu', 'residual_connection': res}
            if variant == 'C':
...
df.to_csv('/Users/yukang/Desktop/NeuroMiss/results/exp_residual.csv', index=False)
