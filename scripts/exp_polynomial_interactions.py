"""Focused experiment: Variant D (polynomial interactions) for nonlinear responses.
This is designed specifically for the meeting notes' polynomial extension case."""
...
# Variant D with different depths and activations
for depth in [2, 3, 5, 7]:
    for act in ['gelu', 'relu', 'tanh']:
        methods.append({'name': f'NM+D_{act}_d{depth}', 'variant': 'D', 'depth': depth, 'activation': act,
                       'degree': 2, 'residual_connection': False})
# Compare with Variant A (no polynomial) as control
for depth in [3, 5]:
    methods.append({'name': f'NM+A_gelu_d{depth}', 'variant': 'A', 'depth': depth, 'activation': 'gelu', 'residual_connection': False})
...
df.to_csv('/Users/yukang/Desktop/NeuroMiss/results/exp_polynomial.csv', index=False)
