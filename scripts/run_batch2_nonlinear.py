"""Batch 2: Non-linear response experiments - Gaussian X + polynomial/nonlinear f.
This is the "easy extension" from the meeting notes."""
import sys
sys.path.insert(0, '/Users/yukang/Desktop/NeuroMiss/src')

from data_generation import DataScenario
from experiment_runner import run_experiment_suite, analyze_results

scenarios = [
    DataScenario('gaussian', 'quadratic', 'MCAR', n_features=10, missing_rate=0.5, snr=10),
    DataScenario('gaussian', 'cubic', 'MCAR', n_features=10, missing_rate=0.5, snr=10),
    DataScenario('gaussian', 'sinusoidal', 'MCAR', n_features=10, missing_rate=0.5, snr=10),
    DataScenario('gaussian', 'interaction', 'MCAR', n_features=10, missing_rate=0.5, snr=10),
    DataScenario('gaussian', 'quadratic', 'MAR', n_features=10, missing_rate=0.5, snr=10),
    DataScenario('gaussian', 'cubic', 'MNAR_censoring', n_features=10, missing_rate=0.5, snr=10),
]

methods = [
    {'name': 'NeuMiss_d1', 'variant': 'original', 'depth': 1, 'activation': 'relu', 'residual_connection': False},
    {'name': 'NeuMiss_d3', 'variant': 'original', 'depth': 3, 'activation': 'relu', 'residual_connection': False},
    {'name': 'NeuMiss_d5', 'variant': 'original', 'depth': 5, 'activation': 'relu', 'residual_connection': False},
    {'name': 'NeuMiss+A_gelu_d3', 'variant': 'A', 'depth': 3, 'activation': 'gelu', 'residual_connection': False},
    {'name': 'NeuMiss+A_relu_d3', 'variant': 'A', 'depth': 3, 'activation': 'relu', 'residual_connection': False},
    {'name': 'NeuMiss+A_tanh_d3', 'variant': 'A', 'depth': 3, 'activation': 'tanh', 'residual_connection': False},
    {'name': 'NeuMiss+A_silu_d3', 'variant': 'A', 'depth': 3, 'activation': 'silu', 'residual_connection': False},
    {'name': 'NeuMiss+B_gelu_d3', 'variant': 'B', 'depth': 3, 'activation': 'gelu', 'residual_connection': False},
    {'name': 'NeuMiss+C_gelu_d3', 'variant': 'C', 'depth': 3, 'activation': 'gelu', 'residual_connection': False, 'expansion_factor': 2},
    {'name': 'NeuMiss+D_gelu_d3', 'variant': 'D', 'depth': 3, 'activation': 'gelu', 'residual_connection': False, 'degree': 2},
    {'name': 'NeuMiss+A_gelu_d5', 'variant': 'A', 'depth': 5, 'activation': 'gelu', 'residual_connection': False},
    {'name': 'NeuMiss+D_gelu_d5', 'variant': 'D', 'depth': 5, 'activation': 'gelu', 'residual_connection': False, 'degree': 2},
]

print("=" * 60)
print("BATCH 2: NON-LINEAR - Gaussian X + Polynomial/Nonlinear f")
print("=" * 60)

df = run_experiment_suite(
    scenarios, methods,
    n_train=10000, n_val=2000, n_test=5000,
    n_epochs=150, batch_size=64, n_repeats=3,
    output_file='/Users/yukang/Desktop/NeuroMiss/results/batch2_nonlinear.csv')

analyze_results(df)
